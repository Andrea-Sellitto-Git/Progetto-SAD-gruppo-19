...
/**
 * Comando per copiare multiple shape nel clipboard. Supporta sia la copia di
 * una singola shape che di un gruppo di shape.
 *
...
public class MultiCopyCommand implements Command {
    ...
    /**
     * Esegue la copia delle shape nel clipboard. Se è presente una sola shape,
     * la copia nel clipboard standard. Se sono presenti multiple shape,
     * gestisce la copia multipla.
     */
    @Override
    public void execute() {
        if (shapes.isEmpty()) {
    ...
    /**
     * Gestisce la copia di multiple shape. Per ora, copia la prima shape nel
     * clipboard standard e mantiene un riferimento interno per le altre (per
    ...
    /**
     * Interfaccia estesa per clipboard che supporta multiple shape. Può essere
     * implementata in futuro per gestire nativamente multiple shape.
     */
    ...
    /**
     * Manager temporaneo per gestire clipboard multiplo. Questa è una soluzione
     * temporanea fino a quando il ClipboardReceiver non viene esteso per
     * supportare nativamente multiple shape.
     */
